...
ggplot(data = DF, aes(x = x, y = px)) + facet_grid(pi~.) + 
  ...
  labs(y = "Probability", x = "$x$", title = "$X \\sim Bin(n = 10, \\pi)$\n")
theme_set(previous_theme) # Restore original theme
...
ggplot(data = DF, aes(x = x, y = fx)) + geom_area(fill = "blue") + facet_grid(l~a) + 
  labs(x = "x", y = "f(x)", title = "X  ~ Gamma(alpha, lambda)") + 
  theme(axis.title.y=element_text(vjust = 0.2)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) 
theme_set(previous_theme) # Restore original theme  


# The Nomral Distribution
par(mfrow = c(1, 3))
# First
...
curve(dnorm(x, 0, 1), xlim = c(-3, 3), ylab = "Normal(0,1)", xlab = "Range")
...
attach(SCORE)
hist(scores, freq = F)
eda(scores) # Incredibly powerful command in R for exploratory 
detach(SCORE)
